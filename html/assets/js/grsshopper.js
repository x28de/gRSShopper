...
function login_box() {		
  if (getCookie(title_cookie) == null) {
   ...
   } else if (getCookie(title_cookie) == "anymouse") {
	document.write("You are not logged in. [<A HREF='" + cgi_url + "login.cgi?refer=" +
	   document.location + "' class='Nav' target='_top'>Login</A>] " +
	   "[<A HREF='" + cgi_url + "login.cgi?action=Register&refer=" +
	   document.location + "'  class='Nav' target='_top'>Register</A>]"); 
   } else {
	document.write("You are logged in as <b>" + getCookie(title_cookie) + "</b> " +
...
function comment_login_box() {
   ...
   var logged = "You are logged on as <b>" + getCookie(title_cookie) + "</b>. You can <a href='" + cgi_url + 
...
$('.arrowup').click(function(event) {
  var n=event.target.id.split("-",2);
  var vid = n[1];
  var request = $.ajax({
    url: "http://www.downes.ca/cgi-bin/page.cgi",
    type: "POST",
    data: {action: "apiv",vote_post:n[1],vote_value: n[0]},
    dataType: "html",
    success: function(response) { 
       var container = document.getElementById('votescore-'+vid);
