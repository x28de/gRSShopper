...
sub run_sql_file {
	...
	close SQLFILE;
      ...
      print &printlang("Database initializaton failed",$dbh::errstr),"\n" if $dbh::err;
      }
...
#print $stmt."<p>";
	my $sthl = $dbh->prepare($stmt);
	$sthl->execute();
		if ($sthl->errstr) { print "DB LIST ERROR: ".$sthl->errstr." <p>"; }
	$output .=  "<p>\n";
...
sub edit_record {
	...
	my $showcols = {
		...
		media => ["title","url","type","mimetype","link","feed","medialink","size","description","submit"],
