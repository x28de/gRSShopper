...
sub new {
    ...
    my($format) = @_;
    $format ||= 'Atom';
    my $format_class = 'Feed::Format::' . $format;
...
sub convert {
    ...
    for my $field (qw( title link content summary category author id issued modified lat long )) {
        ...
        next if blessed $val && $val->isa('Feed::Content') && ! defined $val->body;
        $new->$field($val);
    }
    $new;
}
...
Feed::Entry - Entry/item in a syndication feed

=head1 SYNOPSIS

    ## $feed is an Feed object.
...
I<Feed::Entry> represents an entry/item in an I<Feed> syndication
feed.

=head1 USAGE

=head2 Feed::Entry->new($format)

Creates a new I<Feed::Entry> object in the format I<$format>, which
should be either I<RSS> or I<Atom>.

=head2 $entry->convert($format)

Converts the I<Feed::Entry> object into the I<$format> format, and
...
Bn I<Feed::Content> object representing the full entry body, or as
...
An I<Feed::Content> object representing a short summary of the entry.
Possibly.
...
an I<Feed::Content> object with an empty string in the I<body>.
...
Take an entry in its native format and turn it into an I<Feed::Entry> object.

=head2 $entry->unwrap

Take an I<Feed::Entry> object and turn it into its native format.

=head1 AUTHOR & COPYRIGHT

Please see the I<Feed> manpage for author, copyright, and license
information.
