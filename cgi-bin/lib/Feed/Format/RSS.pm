...
sub init_empty {
    ...
    $feed->{rss}->add_module(prefix => "event", uri => 'http://www.downes.ca/event/specification');
    $feed;
}
...
sub init_string {
    ...
    $opts->{allow_multiple} = [ 'enclosure' ] if $Feed::MULTIPLE_ENCLOSURES;
    if ($str) {
...
sub grog       { shift->{rss}->channel('grog', @_) }
sub grog2       { shift->{rss}->channel('grog2', @_) }
...
sub entries {
    ...
    for my $item (@{ $rss->{items} }) {
        push @entries, Feed::Entry::Format::RSS->wrap($item);
...
sub summary {
    my $item = shift->{entry};
    if (@_) {
        $item->{description} = ref($_[0]) eq 'Feed::Content' ?
            $_[0]->body : $_[0];
        ...
        Feed::Content->wrap({ type => 'text/plain', body => $txt });
    }
}
...
sub content {
    ...
    if (@_) {
	  ...
	  # Stephen - 03March2011 - to get full-length contents from Wordpress
	  $item->{content}{encoded} = $body;

        Feed::Content->wrap({ type => 'text/html', body => $body, base => $base });
...
sub enclosure {
    ...
    if (@_) {
        ...
        my @encs = map { Feed::Enclosure->new($_) } (ref $tmp eq 'ARRAY')? @$tmp : ($tmp);
